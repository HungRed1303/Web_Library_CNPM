  ...
  class LibraryCardRequestService {
    ...
    async getAllLibraryCards() {
      try {
        const response = await this.axiosInstance.get('/librarycard');
        if (response.data.success) {
          return response.data.data;
        } else {
          throw new Error(response.data.message || 'Failed to fetch library cards');
        }
      } catch (error) {
    ...
    // Approve a library card request
    async approveLibraryCardRequest(studentId) {
      try {
        const response = await this.axiosInstance.put(`/librarycard/approve/${studentId}`);
        if (response.data.success) {
          return response.data.data;
        } else {
          throw new Error(response.data.message || 'Failed to approve library card request');
        }
      } catch (error) {
        console.error('Error approving library card request:', error);
        throw error;
      }
    }
    // Reject a library card request
    async rejectLibraryCardRequest(studentId) {
      try {
        const response = await this.axiosInstance.put(`/librarycard/reject/${studentId}`);
        if (response.data.success) {
          return response.data.data;
        } else {
          throw new Error(response.data.message || 'Failed to reject library card request');
        }
      } catch (error) {
        console.error('Error rejecting library card request:', error);
        throw error;
      }
    }
